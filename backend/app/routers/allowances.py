...
def create_allowance(data: AllowanceCreate, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("allowances", "create"))):
    return allowance_service.create_allowance(db, data, current_user.id)

@router.get("", response_model=List[AllowanceResponse])
def list_allowances(db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("allowances", "view"))):
    return allowance_service.get_allowances(db, current_user.id)

@router.get("/{allowance_id}", response_model=AllowanceResponse)
def get_allowance(allowance_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("allowances", "view"))):
...
def update_allowance(allowance_id: int, data: AllowanceCreate, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("allowances", "edit"))):
...
def delete_allowance(allowance_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("allowances", "delete"))):
