...
def create_allowance(data: AllowanceCreate, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_ALLOW_WRITE))):
    return allowance_service.create_allowance(db, data, current_user.id)

@router.get("", response_model=List[AllowanceResponse])
def list_allowances(db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_ALLOW_READ))):
    return allowance_service.get_allowances(db, current_user.id)

@router.get("/{allowance_id}", response_model=AllowanceResponse)
def get_allowance(allowance_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_ALLOW_READ))):
...
def update_allowance(allowance_id: int, data: AllowanceCreate, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_ALLOW_WRITE))):
...
def delete_allowance(allowance_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_ALLOW_DELETE))):
