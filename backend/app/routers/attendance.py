...
def list_attendance_all(
    ...
    current_user: User = Depends(require_perm(PERM_ATTEND_READ)),
):
    return attendance_service.get_all_attendance(db, current_user.id, guard_id)

@router.post("", response_model=AttendanceResponse, status_code=status.HTTP_201_CREATED)
def create_attendance(data: AttendanceCreate, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_ATTEND_WRITE))):
