...
@router.get("/receipts")
def list_receipts(db: Session = Depends(get_db), current_user: User = Depends(require_module("billing", "view"))):
    rows = stripe_svc.list_billing_receipts(db, current_user)
    return [stripe_svc.billing_receipt_out(r) for r in rows]


@router.get("/receipts/{receipt_id}")
def get_receipt(receipt_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_module("billing", "view"))):
