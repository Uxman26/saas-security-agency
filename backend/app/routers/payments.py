...
def list_payments(invoice_id: Optional[int] = None, db: Session = Depends(get_db), current_user: User = Depends(require_module("payments", "view"))):
    return payment_service.get_payments(db, current_user.id, invoice_id)

@router.get("/{payment_id}", response_model=PaymentResponse)
def get_payment(payment_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_module("payments", "view"))):
    return payment_service.get_payment(db, payment_id, current_user.id)

@router.put("/{payment_id}", response_model=PaymentResponse)
def update_payment(payment_id: int, data: PaymentUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_module("payments", "edit"))):
    return payment_service.update_payment(db, payment_id, data, current_user.id)

@router.delete("/{payment_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_payment(payment_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_module("payments", "delete"))):
