...
def create_payroll(data: PayrollCreate, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_PAYROLL_WRITE))):
    return payroll_service.create_payroll(db, data, current_user.id)

@router.post("/calculate", response_model=PayrollResponse)
def calculate_payroll(guard_id: int, period_start: date, period_end: date, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_PAYROLL_WRITE))):
...
def calculate_payroll_batch(body: PayrollBatchRequest, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_PAYROLL_WRITE))):
...
def list_payrolls(guard_id: Optional[int] = None, period_start: Optional[date] = None, period_end: Optional[date] = None, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_PAYROLL_READ))):
