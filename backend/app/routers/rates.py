...
from app.rbac import require_perm, PERM_RATES_READ, PERM_RATES_WRITE, PERM_RATES_DELETE
...
def create_guard_rate(guard_id: int, data: GuardRateCreate, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_RATES_WRITE))):
...
def list_guard_rates(guard_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_RATES_READ))):
...
def delete_guard_rate(rate_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_RATES_DELETE))):
...
def create_site_rate(site_id: int, data: SiteRateCreate, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_RATES_WRITE))):
...
def list_site_rates(site_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_RATES_READ))):
...
def delete_site_rate(rate_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_RATES_DELETE))):
