...
def dashboard_stats(db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "view"))):
    return report_service.get_dashboard_overview(db, current_user.id)


@router.get("/compliance", response_model=List[ComplianceAlert])
def compliance_alerts(days: int = 30, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "compliance_view"))):
...
def contracts_expiring(days: int = 30, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "contracts_expiring_view"))):
    return report_service.get_contract_expiry_alerts(db, current_user.id, days)


@router.get("/hub", response_model=ReportsHubResponse)
def reports_hub(start_date: date, end_date: date, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "view"))):
...
def staff_shift_hours(
    ...
    current_user: User = Depends(require_internal_module("reports", "staff_reports")),
):
...
def staff_monthly(start_date: date, end_date: date, group_by: str = "guard", db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "staff_reports"))):
...
def staff_individual(guard_id: int, start_date: date, end_date: date, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "staff_reports"))):
...
def attendance_report(start_date: date, end_date: date, guard_id: Optional[int] = None, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "attendance_reports"))):
...
def shift_overtime_report(start_date: date, end_date: date, guard_id: Optional[int] = None, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "shift_variance_reports"))):
...
def shift_early_finish_report(start_date: date, end_date: date, guard_id: Optional[int] = None, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "shift_variance_reports"))):
...
def financial_invoices(start_date: date, end_date: date, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "financial_reports"))):
...
def subscription_summary(db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "subscription_reports"))):
...
def subscription_invoices(start_date: date, end_date: date, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "subscription_reports"))):
    return reports_extended_service.subscription_invoice_rows(db, current_user.id, start_date, end_date)


@router.get("/usage/logins")
def usage_logins(start_date: date, end_date: date, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "usage_reports"))):
    return reports_extended_service.login_report_rows(db, current_user.id, start_date, end_date)


@router.get("/usage/summary", response_model=UsageSummaryResponse)
def usage_summary(start_date: date, end_date: date, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("reports", "usage_reports"))):
