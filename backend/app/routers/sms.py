...
def patch_config(body: SmsConfigUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_EMAIL_SEND))):
...
def send_sms(body: SmsSendRequest, db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_EMAIL_SEND))):
...
def sms_logs(db: Session = Depends(get_db), current_user: User = Depends(require_perm(PERM_EMAIL_SEND))):
