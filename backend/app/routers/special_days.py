...
def list_special_days(
    ...
    current_user: User = Depends(require_internal_module("allowances", "view")),
):
...
def create_special_day(
    ...
    db: Session = Depends(get_db),
    current_user: User = Depends(require_internal_module("allowances", "edit")),
):
...
def seed_uk_holidays(
    body: SeedUkYear,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_internal_module("allowances", "seed_uk")),
):
...
def delete_special_day(
    day_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_internal_module("allowances", "delete")),
):
