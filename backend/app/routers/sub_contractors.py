...
def create_sub_contractor(sub_contractor: SubContractorCreate, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("sub_contractors", "create"))):
...
def get_sub_contractors(
    ...
    current_user: User = Depends(require_internal_module("sub_contractors", "view")),
):
    return sub_contractor_service.get_sub_contractors(db, current_user.id, main_contractor_id=main_contractor_id)

@router.get("/{sub_contractor_id}", response_model=SubContractorResponse)
def get_sub_contractor(sub_contractor_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("sub_contractors", "view"))):
...
def update_sub_contractor(sub_contractor_id: int, sub_contractor: SubContractorCreate, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("sub_contractors", "edit"))):
...
def delete_sub_contractor(sub_contractor_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_internal_module("sub_contractors", "delete"))):
