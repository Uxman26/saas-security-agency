...
def get_subscription(db: Session = Depends(get_db), current_user: User = Depends(require_module("billing", "view"))):
    return subscription_service.get_subscription(db, current_user.id)

@router.put("", response_model=CompanyResponse)
def update_subscription(data: SubscriptionUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_module("billing", "edit"))):
