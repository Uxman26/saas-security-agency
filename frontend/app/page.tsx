...
const FEATURE_KEYS = ['guards', 'sites', 'rota', 'payroll', 'invoices', 'compliance', 'subs', 'rates', 'dashboard'] as const;
const FEATURE_ICONS = [Users, MapPin, Calendar, Wallet, FileText, AlertTriangle, UserCog, ClipboardList, BarChart3];
const STAT_KEYS = ['platform', 'sia', 'compliant', 'scalable'] as const;
const STEP_KEYS = ['plan', 'signup', 'live'] as const;
const FAQ_KEYS = ['plan', 'secure', 'sia', 'trial'] as const;
    ...
    <div className="min-h-screen bg-background">
            ...
            <ScrollReveal>
              <div className="mb-6 flex flex-col items-center gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-lg font-semibold text-primary">{t('tagline')}</p>
                </div>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal delay={320}>
              <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
                <Button asChild size="lg" className="gap-2 shadow-lg shadow-primary/20">
                  <Link href="/pricing">{tc('viewPlans')} <ArrowRight className="size-4 rtl:rotate-180" /></Link>
                </Button>
                <Button asChild variant="outline" size="lg" className="backdrop-blur-sm bg-background/50">
                  <Link href="/pricing">{tc('createCompany')}</Link>
                </Button>
              </div>
            </ScrollReveal>
          </div>
            ...
            {STAT_KEYS.map((key, i) => (
              <ScrollReveal key={key} delay={i * 80}>
                <div className="text-center rounded-2xl border border-border/60 bg-card/60 backdrop-blur-sm p-5 transition-all hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-0.5">
                  <p className="text-xl font-bold text-primary md:text-2xl">{t(`stats.${key}.value`)}</p>
              ...
              <ScrollReveal key={key} delay={i * 60}>
                <Card className="h-full border-border/70 bg-card/90 transition-all duration-300 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1">
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription className="text-base">{t(`features.${key}.desc`)}</CardDescription>
                  </CardHeader>
                </Card>
              </ScrollReveal>
            );})}
            ...
            {STEP_KEYS.map((key, i) => (
              <ScrollReveal key={key} delay={i * 120}>
                <div className="relative text-center group">
                  ...
                  <h3 className="text-xl font-semibold text-foreground">{t(`steps.${key}.title`)}</h3>
                  <p className="mt-2 text-muted-foreground">{t(`steps.${key}.desc`)}</p>
                </div>
              </ScrollReveal>
            ))}
          </div>
          ...
          <ScrollReveal>
            <Card className="mx-auto max-w-3xl border-primary/20 bg-gradient-to-br from-primary/5 via-card/80 to-violet-500/5 backdrop-blur-sm overflow-hidden">
              ...
              <CardContent className="relative p-8 md:p-12">
                ...
                <p className="mt-4 font-medium text-foreground">{t('quoteAuthor')}</p>
              </CardContent>
            </Card>
          </ScrollReveal>
        </div>
              ...
              <ScrollReveal>
                <div className="group rounded-2xl border border-border/60 bg-card/50 p-6 transition-all hover:border-primary/25 hover:shadow-lg">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-semibold text-foreground">{t('complianceTitle')}</h3>
                  </div>
              ...
              <ScrollReveal delay={120}>
                <div className="group rounded-2xl border border-border/60 bg-card/50 p-6 transition-all hover:border-primary/25 hover:shadow-lg">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-semibold text-foreground">{t('fastTitle')}</h3>
                  </div>
                  <p className="text-muted-foreground leading-relaxed">
                    {t('fastDesc')}
                  </p>
          ...
          <ScrollReveal>
            <Card className="mx-auto max-w-4xl border-primary/20 bg-primary/5 backdrop-blur-sm">
              <CardHeader className="text-center pb-2">
                ...
                <CardTitle className="text-2xl md:text-3xl">{t('pricingTitle')}</CardTitle>
              ...
              <CardContent className="flex flex-col items-center gap-8 pt-6">
                ...
                <div className="flex flex-wrap items-center justify-center gap-4">
                  <Button asChild size="lg" className="shadow-md shadow-primary/15">
                    <Link href="/pricing">{tc('seePlans')}</Link>
                  </Button>
                  <Button asChild variant="outline" size="lg">
                    <Link href="/pricing">{tc('signUp')}</Link>
                  </Button>
                </div>
              </CardContent>
            </Card>
            ...
            {FAQ_KEYS.map((key, i) => (
              <ScrollReveal key={key} delay={i * 80}>
                <div className="rounded-xl border border-border/70 bg-card/80 backdrop-blur-sm p-6 transition-all hover:border-primary/20 hover:shadow-md">
                  <h4 className="font-semibold text-foreground">{t(`faqs.${key}.q`)}</h4>
                  <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{t(`faqs.${key}.a`)}</p>
                </div>
              </ScrollReveal>
            ))}
          </div>
      ...
      <section className="py-20 md:py-28">
        <ScrollReveal className="container mx-auto px-4 text-center">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
            <Button asChild size="lg" className="gap-2 shadow-lg shadow-primary/20">
              <Link href="/pricing">{tc('createCompany')} <ArrowRight className="size-4 rtl:rotate-180" /></Link>
            </Button>
            <Button asChild variant="outline" size="lg">
              <Link href="/about">{tc('learnMore')}</Link>
            </Button>
          </div>
        </ScrollReveal>
      </section>
