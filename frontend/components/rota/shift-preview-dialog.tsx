...
import { fmtShortDate, formatHoursDecimal, shiftSiteLine } from '@/lib/rota-shifts-utils';
import type { EmployeeRec, RotaJsState, ShiftRec } from '@/lib/rota-shifts-types';
import { calcHours } from '@/lib/rota-shifts-utils';
...
function buildShiftMessage(employee: EmployeeRec, state: RotaJsState, rotaName: string): string {
...
export function ShiftPreviewDialog({ open, onOpenChange, employee, state, rotaName }: Props) {
  ...
  const totalHours = shiftsByDay.reduce(
    (sum, { dk, shifts }) =>
      sum + shifts.reduce((s, sh) => s + calcHours(sh, state.inclBreaks), 0),
    0
  );
  ...
  const sendWhatsApp = () => {
    ...
    const msg = buildShiftMessage(employee, state, rotaName || state.rotaName || 'Rota');
